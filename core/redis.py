...
async def add_to_room(room_id, client_id):
    await redis_client.hset(ROOMS_KEY_TEMPLATE.format(room_id=room_id), client_id, "")

async def remove_from_room(room_id, client_id):
    await redis_client.hdel(ROOMS_KEY_TEMPLATE.format(room_id=room_id), client_id)

async def get_room_clients(room_id):
    return list(await redis_client.hkeys(ROOMS_KEY_TEMPLATE.format(room_id=room_id)))
